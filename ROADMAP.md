    ...
    * реализация
      ...
      * единый центр запуска для видимых и невидимых блоков
        ...
        * не создавать каждый раз для запуска кода новую виртуальную машину без надобности
          ...
          * а по хорошему у каждого run block'а должна быть своя VM, которая пересоздаётся только при изменении кода
          * при этом старые VM должны сохраняться на случай, если run block'у вернут предыдущий код
        * главный эффект выражается в том, что если next переходит на невидимый knoxel, то его knyte всё равно запускается
        * дополнительно к этому каким-то образом должен блокироваться повторный запуск knyte'а до завершения выполнения его кода
          * а может быть не блокироваться, а разруливаться другим способом - вобщем это надо обдумать
            * ситуация, когда код блока ещё не закончил выполнение, а его снова пытаются запустить вполне реальна
  * условные переходы между связками
    * спецификации на input и output параметры
    * прототип if else
    * прототип switch case
    * прототипы while и/или for, если в них есть смысл при наличии if else
  * тестовые задачи
