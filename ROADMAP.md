  ...
  * information record
    ...
    * реализация отображения контента, заданного из вне через html-код
      ...
      * контекстное редактирование контента в foreignObject'ах и назначение цвета rect'ам
        ...
        * принять нешение о дальнейшей разработке редакторов
          * в конечном счёте редакторы должны быть реализованы через интерактивные knoxel'и
          * но пока система не достаточно развита для этого, нужно реализовать сторонние редакторы для базовых нужд
        * редакторы information record'ов
          * `общая система определения типа контента и привязывания к нему соответствующего редактора`
            * в случае текстовых редакторов исходник текста не равен итоговому html-коду
            * record должна содержать не только итоговый код, но и обозначение редактора и его входные данные
              * на базе которых был создан итоговый код
            * если никаких данных о редакторе нет, значит мы имеем дело с чистым кодом
            * редактор кода может брать на редактирование содержимое записей, сделанных в других редакторах
            * другие редакторы просто перезаписывают содержимое knyte'ов с чистым кодом или редакторов другого типа
              * потому что реализовывать обратный парсинг - это путь в никуда
          * на уровне простого однострочного центрированного текста
            * с автоматическим определением ширины rect'а
            * вызывается по alt+enter
            * представляет из себя простой prompt с текстовой строкой
          * на уровне простого многострочного центрированного текста
            * с поддержкой форматирования через multiple spaces, \t, \n
            * с автоматическим определением высоты rect'а при заранее заданной ширине
            * вызывается по shift+enter
            * кастомное модальное окно с textarea или даже с нормальным текстовым кода (ace/codemirror/monaco)
          * редактор information record'ов на уровне текста, который интерпретируется как хтмл-код
            * вызывается по cmd+enter
            * кастомное модальное окно с textarea или даже с нормальным редактором кода (ace/codemirror/monaco)
        * редактор размера
          * вызывается по cmd+space
          * представляет из себя простой prompt с кодом вида {w: 100, h: 50}
        * редактор цвета
          * вызывается по alt+space
    ...
    * установка шрифта Meslo по умолчанию для всего веб-приложения
  * vector
    * в том числе добавить стрелки в space map, чтобы показать связность пространств в островах
      * а для этого предварительно придётся перевести описание самой системы пространств на векторы
