...
* работа с группами knoxel'ей
  ...
  * реализация
    ...
    * упаковка knoxel'ей накрываемых рамкой в новый knoxel
      * начало рисования рамки по alt+shift+click
      * отмена рисования рамки по esc
      * применение рамки по click
* сделать альтернативное управление steering'ом для Windows и Linux
  * так как на них обычно нет удобного тачпада и нужно справляться мышью
    * в итоге для pan'а нужно водить мышью с зажатым колесом или средней кнопкой
    * масштабирование оставил на shift + wheel
* упорядочить сохранение данных app strate'а
  * оптимизировать сохранение для больших knyte cloud'ов
    * оказывается, что сериализация в json с сортировкой ключей очень сильно тормозит
      * в итоге разделил созранение в файл на 2 варианта: с сортировкой и без сортировки ключей
        * можно применять оптимальный вариант в зависимости от ситуации
  * навести порядок в именовании файлов
    * выбрать knoxelSpace[.min].json или knyte-appstate[.min].json для всех вариантов сохранения
      * в итоге везде используется knyte-appstate[.min].json
* выгрузка результатов расчётов logic-блока в output-параметр solution
   * для дальнейшего использования в логике программ
* кастомная интерактивность knoxel'ей
  * кастомизация пользовательского ввода
    * отключаемый клик на вложенные knoxel'и и сам space root
    * отключаемые кнопки - все кроме системных и навигации между пространствами
  * ввести мапу состояния кнопок через keydown/keyup для игровых циклов
  * custom block для управления кастомизируем поведением внутри space'а knoxel'я
    * базовая спецификация на основные фичи custom block'ов в core knyte appstate
    * ввести специальный api для доступа к избранным системным стейтам и функциям
    * базовая реализация
* управление knoxel'ями через api
  * состояние зажатых клавиш
  * изменение цвета обводки
  * изменение позиции в host space'е
  * collapse/expand
* управление knyte'ами за пределами host space'а
  * запись данных в knyte
    * пока что только на уровне oneliner'а
* оценка путей устранения копипаста в коде
  * на данный момент запуск run block'ов реализован в 3 вариациях:
    * клиентская (браузер)
    * heroku
    * netlify functions
  * запуск run block'ов состоит из 3 основных частей
    * загрузка аппстейта из файла/gist/repo
    * хранение базового аппстейта
    * непосредственно запуск run block'ов в текущем аппстейте
  * в итоге нужно создать универсальную реализацию, которая будет работать как для браузера, так и для netlify functions
* удалить поддержку деплоя на heroku
  * так как netlify удобнее и вполне справляется со этой работой
* перевести хранение персистентных данных с gist на git repos
  * цель данной реализации
    * повышение уровня надёжности хранения секретных данных в knyte appstate'е
  ...
  * спроектировать оптимальное решение
    * в итоге хранение файлов appstate'ов в отдельных репах оказалось оптимальным решением
  * реализация
    * переделать страницу connect с gist id и optional PAT на owner, repo и required PAT
    * переделать вызов апи гитхаба для чтения данных с gist на contents
      * для браузера
      * для лямбда-функции
      * методика чтения appstate'а из приватного репа
      * методика чтения большого appstate'а (более 1 Мб)
        * для браузера
          * основная идея в том, что надо работать не по пути к файлу, а по sha
            * для этого надо раскрутить цепочку: repo -> commit -> tree -> file -> blob -> final text content
        * для лямбда-функции
      * методика передачи своих публичных репов другим людям только для чтения
        * когда они используют собственный PAT для доступа к содержимому репы
    * переделать вызов апи гитхаба для сохранения данных с gist на contents
      * добавить putRepoFile в connection.js для записи в файл
      * заменить старые вызовы для записи в gist на новые
    * обязательная сквозная авторизация в github через сервер, чтобы не упираться в анонимные лимиты апи коллов
      * 60 вызовов в час без авторизации на весь сервак против 5000 вызовов в час с авторизацией на каждого клиента
      * соответственно убрать process.env.githubPAT из приложения Netlify
    * оформить актуальные аппстейты в репы и скорректировать их код по необходимости
      * тестовые публичный и приватный репозитории третьего лица
      * мои личные тестовые публичный и приватный репозитории
      * спека knyte.io
    * привести в порядок локальную insomnia
    * обновить readme
    * удалить остатки кода для gist'ов и fetchRepoStatus_Less1Mb
* разобраться что делать с багом когда fetchRepoStatus не находит файл appstate
  * метод сделан несколько некорректно и смотрит не актуальный tree, а files последнего commit'а
    * поэтому если в последнем коммите нет файла appstate'а, то он не подключается к репе
    * текущий workaround - держать в репе только файл appstate'а
  * нужен ли фикс и расширение возможностей данной связки - вопрос открытый
    * да, фикс нужен, потому что как минимум могут быть пустые коммиты
      * плюс может хранение дополнительных файлов рядом с аппстейтом может быть полезным
  * фикс сделан, теперь привязка осуществляется к актуальному tree
* реализовать защиту от перезаписи версий
  * предотвращать перезапись appstate'ов с потерей данных из разных инстансов knoxel space'а
    * при загрузке appstate'а сохранять загруженную ревизию
    * перед сохранением appstate'а проверять, что последняя ревизия равна загруженной
      * если это не так, то запрещать сохранение данных 
        * так как за это время появилась новая ревизия/несколько ревизий файла аппстейта
  * разобраться с проблемой когда после обновления страницы она грузит старую версию appstate'а из репы
    * в браузере
    * на сервере
  * ограничение на объём загрузки - 64 Мб
    * чтобы не грузили в репу слишком большие файлы и не забывали про очистку кешированных файлов
* телепортирование к указанным элементам по id
  * jumpknyte - фокус должен оказаться в пространстве указанного knyte'а
    * по итогам попытки реализации стало понятно, что такая функция не имеет смысла
  * jumpknoxel - фокус должен оказаться на изображении указанного knoxel'я
    * независимо от того, находится knoxel в текущем выбранном пространстве или вне него
* доработка базовых функций навигации и управления визуализацией
  * основная идея в том, чтобы оставаться в концепции фрактального пространства
    * а не пользоваться наследием старины в виде кнопок в скринспейсе или всплывающих окон
    * по необходимости можно расширить саму концепцию фрактального пространства
  * сброс навигации в пространстве на случай если пользователь в нём потерялся
     * KeyR - reset navigation
  * пересмотреть пункты по навигации и удалению с учётом новой цели майлстоуна-3
  * список новых инструментов навигации на реализацию
    * базовый механизм для навигационных подпространств
      * при нажатии условленной кнопки на выбранном knoxel'е для него создаётся навигационное подпространство
      * оно размещается прямо поверх выбранного knoxel'я, и затем его можно передвинуть через ghost
    * knoxelmap - карта knoxel'ей привязанных к данному knyte'у
      * выглядит как подпространство для knyte'а с jumpable референсами на все его knoxel'и. при этом оно не содержит ни одного дополнительного knoxel'я для данного knyte'а, а формируется из специально для этого созданных knyte'ов.
      * управляющая клавиша - KeyK
    * knytemap - карта связей данного knyte'а с остальными knyte'ами
      * выглядит как подпространство с набором соединённых knoxel'ей связных knyte'ов
      * слева от knoxel'я выбранного knyte'а - входящие связи, а справа - исходящие
      * управляющая клавиша - KeyL
* `общая система типизированного ввода данных в таблицы с валидацией`
  * генерация формы ввода
  * валидация ввода
  * сообщения об ошибках
  * запись введённых данных в типизированную таблицу
* МАЙЛСТОУН-3: готовность к проведению презентаций по принципам семантического моделирования и программирования для разработчиков
* пересмотреть пункты данного майлстоуна с учётом новых идей и навигационных механизмов
* доработка базовых функций навигации
  * knytemap
    * достраивание частичного knytemap'а для выбранного knoxel'я
      * > (.) - достроить knoxel'и для исходящих связей справа
      * < (,) - достроить knoxel'и для входящих связей слева
  * spacemap
    ...
    * альтернативная реализация
      * выбрать делаем ли мы спец слой для спец пространств или встраиваемся в текущую систему
        * делаем отдельную систему в popup-окне для отрисовки производных view для knyte cloud
          * она не будет входить ни в историю переходов между пространствами, ни в перманентный набор данных
      * заглушки для основных функций - показывать popup-окно с крестиком закрытия поверх spaceRoot по кнопкам
        * KeyI - карта островов (замена для spacemap)
      * отображение островов для KeyI-режима
    * демонтаж старой реализации
* развитие возможностей удаления knoxel'ей
  * цель - удаление knoxel'ей с минимальным количеством ручных манипуляций
  * подумать над механизмом упрощения удаления ненужных групп knoxel'ей 
  * подумать над механизмом складирования/переиспользования ненужных knyte'ов
  * подумать над целесообразностью удаления ненужных knyte'ов
* улучшение интерфейса выбора цвета для knoxel'я
...
* новый уровень работы с объёмными и сложно структурированными текстами
  * назначение
    * более удобно отображать блоки с большим объёмом форматированного текст или html-вёрстки
  * требования
    * автоматически переносить по строкам длинные тексты
    * отображать в редакторе невидимые символы (пробел, табуляция, перевод строки)
    * подсветка кода (html/css/js/json)
  * реализация
    * встроить веб редактор кода в систему
    * создать viewertype'ы на основе редактора кода для реализации всех требований
      * wordwrap
        * должен иметь фиксированную ширину блока, чтобы исходя из неё делать автопереносы слов и определять высоту блока
      * coder'ы, не имеющие функции автопереноса слов
        * htmlcoder
        * csscoder
        * jscoder
        * jsoncoder
* МАЙЛСТОУН-4: разработка клиент-серверных приложений для реальных бизнес-задач
...
* унифицировать способ работы с аппстейтами и run block'ами для browser и netlify functions
  * написать единый код
  * удалить все старые реализации
...
* МАЙЛСТОУН-5: наглядная визуальная разработка комплексных бизнес-приложений
* обработка всех возможных ошибок для custom block'ов
  * понятная индикация состояния custom block'а
  * выживание после любых возможных ошибок в коде и структуре
  * прямые ссылки из консольных сообщений об ошибках на проблемные строчки кода в VM
* дополнить спецификацию custom block'а с учётом новых возможностей
* оптимизация выполнения run block'ов в контексте сложного пространства и массивного семантического облака
  * на данный момент чем больше элементов в сцене, тем дольше выполняется каждый отдельный run block
  * нужно снизить затраты на перерисовку сцены за счёт простых оптимизаций
    * например, когда запись из run block'а производится в collapsed knoxel, то нет необходимости обновлять всю сцену
    * для обновления текста внутри run block'ов (busy.../ready) вполне можно сделать оптимизированный update
      * а не перестраивать каждый раз всю сцену
* реализация custom-программ через механизм run block'ов
* поисковый механизм
  * searchdata - поиск knyte'ов по record.data
  * развитие комплексного языка программирования для поиска, позволяющий искать и по содержимому knyte'ов, и по их структуре связей
* МАЙЛСТОУН-6: наглядная визуальная разработка интерактивных real-time приложений
...
* МАЙЛСТОУН-7: рефлексия на уровне истории изменений и реализации прикладных функций
...
* МАЙЛСТОУН-8: исследование концепции интерактивной базы знаний и семантического языка программирования