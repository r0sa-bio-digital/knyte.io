  ...
  * и всё вместе в любой комбинации
    * разделить size на 2 части:
      ...
      * `knoxel collapse задаётся для конкретного knoxel'я и действует только в его host-пространстве`
        ...
        * определяется в informationMap[knyteId].space[knoxelId].collapse
        * при этом позиция будет определяться в informationMap[knyteId].space[knoxelId].position
          * после интенсивных размышлений пришёл к выводу, что кастомный размер knoxel'я не имеет большого смысла
            * потому что в большинстве случаев хочется просто целиком его схлобнуть
              * зато реализация clipping'а контента для кастомных размеров rect'а сильно усложнит разработку
