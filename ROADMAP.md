...
* расширение интерактивного функционала
  ...
  * подключить универсальное хранение app state'ов для клиента и сервера
    ...
    * ux
      * когда gist загружен без write-доступа, делать его автоматически readonly
        * ввести для редактора режим readonly, когда можно исследовать пространство, но нельзя его изменять
        * применить этот режим для gist'ов на старте
        * подумать как реагировать в редакторе на изменение connection'а в другой табе
          * потому что доступ к gist'у может быть изменён прямо во время его редактирования
      * предотвращать перезапись gist'ов с потерей данных из разных инстансов knoxel space'а
        * скорее всего потребуется держать постоянное и единственное соединение редактируемого инстанса с сервером
          * остальные инстансы должны быть readonly до тех пор, пока редактирующий инстанс не потеряет соединение
  ...
  * написать api call для расчёта корней квадратного уравнения по заданным коэффициентам a, b, c
    * a*x**2 + b*x + c = 0
