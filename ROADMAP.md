  ...
  * vector
    * спроектировать как совмещать rect и arrow отрисовки в рамках одного knoxel'я
      ...
      * реализовать возможность соединять стрелкой указанные knoxel'и
        ...
        * корректное поведение rect'а и arrow во всех режимах
          ...
          * `скрытие rect'а если у knoxel'я указаны параметры vector'а, но при этом отсутствует size/record/record.data`
          * показ rect'а, если с его knoxel'я убирается vector или ему добавляется size/record
          * точное соединение стрелкой других стрелок (rect --> arrow / arrow --> rect / arrow --> arrow)
            * при условии, что у них скрыты rect'ы
            * когда rect показан, стрелка идёт на него и ситуация не отличается от rect --> rect
          * автопозиционирование rect'а по центру стрелки, если она актуальна, а у rect'а задан size/record
          * придумать как интерпретировать ghost-mode для стрелок
          * придумать как интерпретировать bubble-mode для стрелок
          * придумать как входить в подпространство стрелок
          * придумать как входить вызывать редактор record'а для стрелок
          * придумать как отображать selfcontained для стрелок
    * реализовать команду соединания knyte'ов vector'ом
    * реализовать команду отображения связи между knyte'ами на уровне knoxel'ей
    * реализовать на уровне интерфейса с протягиванием на уровень knyte'ов
      * создание связи между knoxel'ями 
      * переназначение связи между knoxel'ями
      * отключение связи между knyte'ами
    ...
    * подумать как делать назначение разных цветов стрелкам
      * по идее цвет стрелки и цвет обводки rect'а должны совпадать
        * поэтому заодно вводится изменяемость цветов обводок
    * подумать о том, что размеры rect'ов и цвета обводок/стрелок могут быть привязаны не к knyte'у а к knoxel'ю
