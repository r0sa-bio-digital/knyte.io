...
* расширение интерактивного функционала
  ...
  * нелинейные конструкции связок
    * когда сразу несколько выполнявшихся параллельно потоков сходятся к одному элементу
      * в этом случае прежде чем переходить дальше
        * run block должен дождаться окончания выполнения всех входящих в него потоков
      * проектирование
        * основная проблема в реализации в том, что не понятно
          * когда нужно сбрасывать состояние аккумулятора переходов
          * run block не знает состояния программы в целом, даже такого понятия сейчас нет
          * решением может быть введение scope'ов через вложение одних run block'ов в другие
            * всё, что внутри run block'а, будет считаться единым scope'ом
            * и на старте можно инициализировать все его аккумуляторы, исходящие переменные и тп.
          * связанные с отсутствие scope'ов проблемы
            * не понятно на какой run block из цепочки жать, чтобы запустить программу
              * имея scope'ы можно будет оформлять в них программы и подпрограммы
                * и иметь единую точку запуска для них
            * одни и те же переменные приходится подключать ко всем run block'ам цепочки
              * если они все оперируют с этими данными
              * а можно подключать переменные к корневому run block'у scope'а
                * и тогда они будут доступны всем run block'ам внутри него
      * реализация
        * `scope как встроенная в run block цепочка вызовов других run block'ов`
        * сброс всех входящих переменных внутри scope'а сразу после запуска корневого run block'а
          * если надо запустить код scope'а без инициализации переменных, нужно запустить первый вложенный run block
        * доступ из вложенных run block'ов ко всем переменным host-scope'а
        * аккумулятор входящих связей от выполненных run block'ов
          * переходит по next только когда получит результаты всех входящих в него веток управления
        * сброс всех аккумуляторов внутри scope'а сразу после запуска корневого run block'а
...
* набор системного функционала
  * серверное выполнение кода
    * идея в том, чтобы запускать логику knyte cloude'а как из браузера, так и через API call'ы
    * это может быть полезно, когда в knoxel space описана некоторая семантика
      * и её надо выдать внешнему сервису для работы
        * это может быть, к примеру, схема базы данных, структурированное описание фактов,
        * соответствие человекочитаемых названий и их машинных идентификаторов, и тп.
    * на первом этапе можно сделать простой запуск единичного run block'а
      * который возвращает значение прикреплённой к нему входящей переменной
        * идентификатор run block'а и название переменной нужно указать в параметрах GET
      * в будущем можно будет перейти к изоморфному коду выполнения логики run block'ов
        * который одновременно работает и в браузере (knoxel space), и на сервере (knyte cloud)
